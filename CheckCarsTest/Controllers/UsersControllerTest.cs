...
public class UsersControllerTests
{
    ...
    private UsersController GetControllerWithUsers(List<IdentityUser> userList)
    {
    ...
    public void GetUsers_ReturnsUsers()
    {
        ...
        var returnedUsers = Assert.IsAssignableFrom<IEnumerable<IdentityUser>>(okResult.Value);
    ...
    public async Task GetUser_ReturnsUser_WhenExists()
    {
        ...
        var controller = GetControllerWithUsers(new List<IdentityUser>());
        controller = new UsersController(userManager.Object, null, null, null);

        var result = await controller.GetUser("1");

        var okResult = Assert.IsType<OkObjectResult>(result);
        var returnedUser = Assert.IsType<IdentityUser>(okResult.Value);
    ...
    {
        var userManager = GetMockUserManager();
        userManager.Setup(m => m.FindByIdAsync("1")).ReturnsAsync((IdentityUser)null);
    ...
    {
        var user = new IdentityUser { Email = "new@example.com" };
        var userManager = GetMockUserManager();
    ...
    {
        var existingUser = new IdentityUser { Id = "1", Email = "old@example.com" };
        var updatedUser = new IdentityUser { Id = "1", Email = "new@example.com", UserName = "newuser" };

        var userManager = GetMockUserManager();
        userManager.Setup(m => m.FindByIdAsync("1")).ReturnsAsync(existingUser);
        userManager.Setup(m => m.UpdateAsync(It.IsAny<IdentityUser>())).ReturnsAsync(IdentityResult.Success);

        var controller = new UsersController(userManager.Object, null, null, null);
        var result = await controller.UpdateUser("1", updatedUser);

        var okResult = Assert.IsType<OkObjectResult>(result);
        var user = Assert.IsType<IdentityUser>(okResult.Value);
